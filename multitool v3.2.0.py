...
try:
    state = Path('guessdata.txt').read_text()
    print(f'"Guess the Number" save data loaded.')
except FileNotFoundError:
    print('"Guess the Number" save data not found.')
    state = Path('guessdata.txt').write_text(repr(0))
    state.replace("", '')
        ...
        if choice == '1':
            ...
            while True:
                ...
                if choice == '1':
                    ...
                    Path('guessdata.txt').write_text(state)
                elif choice == '2':
                    minigames.cargame()
                elif choice == '3':
                    minigames.trivia_game()
                elif choice == 'R':
                    descriptions.re_welcome()
                    break
                else:
